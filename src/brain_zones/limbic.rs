...
impl LimbicSystem {
    pub fn new(n_neurons: usize, config: &Config) -> Self {
        ...
        // Amígdala: mais FP16 para precisão emocional
        let mut amy_dist = config.precision_distribution.clone();
        amy_dist.push((PrecisionType::FP16, 0.55));
        
        // Accumbens: mais INT8 para processamento de recompensa
        let mut acc_dist = config.precision_distribution.clone();
        acc_dist.push((PrecisionType::INT8, 0.65));
        
        let amy = CamadaHibrida::new(n_sub, "limbico_amygdala", Some(amy_dist));
        let acc = CamadaHibrida::new(n_sub, "limbico_accumbens", Some(acc_dist));
    ...
    pub fn evaluate(&mut self, sensory_valence: &[f32], reward_signal: f32, dt: f32, current_time: f32, config: &Config) -> (f32, f32) {
        let mut rng = thread_rng();
        let n = self.amygdala.neuronios.len();

        let mut amy_in = vec![0.0; n];
        for i in 0..n {
            let s = sensory_valence.get(i).cloned().unwrap_or(0.0);
            let intensity = s.abs();
            let sign_weight = if s < 0.0 { 1.5 } else { 0.5 };
            let novelty_bonus = if intensity > 0.7 { 8.0 } else { 0.0 };

            let mut current_input = (intensity * 12.0 * sign_weight) + novelty_bonus;
            current_input += rng.gen_range(-1.0..1.0);
            ...
            amy_in[i] = current_input;
        }
        
        let amy_spikes = self.amygdala.update(&amy_in, dt, current_time);

        let fear_factor = amy_spikes.iter().filter(|&&s| s).count() as f32 / n as f32;
        let fear_inhibition = (1.0 - (fear_factor * 0.5)).max(0.2);

        let acc_base_input = reward_signal * 15.0 * self.dopamine_mod * fear_inhibition;
        let acc_in = vec![acc_base_input; self.nucleus_accumbens.neuronios.len()];
        let acc_spikes = self.nucleus_accumbens.update(&acc_in, dt, current_time);

        let fear_score = fear_factor;
        let pleasure_score = acc_spikes.iter().filter(|&&s| s).count() as f32 / self.nucleus_accumbens.neuronios.len() as f32;

        self.emotional_state = (self.emotional_state * 0.92) + (pleasure_score - fear_score) * 0.6;
        self.emotional_state = self.emotional_state.clamp(-1.0, 1.0);

        self.arousal_level = 0.8 + (fear_score * 2.0) + (pleasure_score * 1.0) + (self.dopamine_mod * 0.2);
        self.arousal_level = self.arousal_level.clamp(0.5, 3.5);
